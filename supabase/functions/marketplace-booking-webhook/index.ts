...
Deno.serve(async(req)=>{
  ...
  const event=JSON.parse(raw) as {provider?:string;eventId?:string;orderId?:string;type?:string;paymentIntentId?:string;providerConfirmation?:string};
  if(!event.provider||!event.eventId||!event.orderId||!event.type)return json({error:'Invalid event'},400);
  const response=await fetch(`${supabaseUrl}/rest/v1/rpc/process_marketplace_booking_webhook`,{method:'POST',headers:{Authorization:`Bearer ${serviceKey}`,apikey:serviceKey,'Content-Type':'application/json'},body:JSON.stringify({p_provider:event.provider,p_provider_event_id:event.eventId,p_order_id:event.orderId,p_event_type:event.type,p_payment_intent_id:event.paymentIntentId??null,p_provider_confirmation:event.providerConfirmation??null})});
