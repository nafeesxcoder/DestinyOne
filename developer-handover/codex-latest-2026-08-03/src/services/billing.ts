...
export type VerifiedStorePurchase = { verified: true; finishedTransactionAllowed: true; entitlements: RestoredEntitlement[] };
...
export async function verifyStorePurchase(input: {
  ...
  requireBillingBackend();
  const { data, error } = await supabase.functions.invoke('verify-store-purchase', { body: input });
  if (error) throw error;
  const payload = data as Partial<VerifiedStorePurchase> & { error?: string } | null;
  if (!payload?.verified || !payload.finishedTransactionAllowed) throw new Error(payload?.error || 'The app store could not verify this purchase. No entitlement was changed.');
  return payload as VerifiedStorePurchase;
}

export async function finalizeStoreRestore(restoreSessionId: string, verifiedCount: number, restoreError: string | null = null) {
  requireBillingBackend();
  const { data, error } = await supabase.functions.invoke('verify-store-purchase', {
    body: { restoreSessionId, finalizeRestore: true, verifiedCount, restoreError },
  });
  if (error) throw error;
  const payload = data as { completed?: boolean; error?: string } | null;
  if (!payload?.completed) throw new Error(payload?.error || 'Restore completion could not be recorded.');
}
