...
## Next.js + Express conversion

The new web delivery lives beside the existing mobile app, so the original Expo build remains untouched:

- `frontend/` — Next.js pages, reusable components, services, hooks, styles, and public assets
- `backend/` — Node.js + Express REST API, signed HTTP-only sessions, rate limiting, Socket.IO chat, and MySQL adapters
- `database/` — database setup notes; the canonical MySQL 8 schema is `backend/schema.sql`
- root Expo files — the existing mobile application

### Run the converted web application
...
Open `http://localhost:3000`. If MySQL credentials are blank, the API runs with clearly marked preview data. For MySQL mode, import `backend/schema.sql`, fill the five `MYSQL_*` values, and replace `JWT_SECRET` with a strong random secret.

Run the conversion quality gate with `pnpm check:conversion`. Set `NEXT_PUBLIC_API_URL` for a deployed API and set `FRONTEND_ORIGIN` to the exact deployed frontend origin.

### Date Marketplace provider setup

The Expo app and converted Next.js delivery now share the same product behavior: profile-city recommendations, category/keyword intent mapping (including hotels and Airbnb-style stays), public-first safety ranking, saved ideas, in-app recommendation notices, and a complete Date Concierge proposal flow. Fresno has verified starter ideas; every other US/Canada city receives useful provider-search results instead of an empty page.

For live business names, hours, ratings and map links, enable **Places API (New)** and set `GOOGLE_PLACES_API_KEY` only in `backend/.env`. The key must never be exposed through `NEXT_PUBLIC_*`. Without the key, the backend returns clearly labelled provider-search cards with Google Maps, Google Travel and Airbnb deep links. Availability and reservation terms remain provider-owned and must be confirmed before booking.

The complete route, folder, verification, and manual launch handoff is documented in [docs/WEB_CONVERSION_DELIVERY.md](./docs/WEB_CONVERSION_DELIVERY.md).
