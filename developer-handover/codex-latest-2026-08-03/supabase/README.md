...
- Expo push token registration, private message notification creation and a
  secret-protected `dispatch-push` Edge Function with invalid-token revocation.
...
- Native Apple/Google purchase preparation, hashed receipt verification audit,
  idempotent entitlement processing and provider-synced restore completion.
- Consent-gated launch sessions/events with an allowlisted funnel taxonomy,
  deletion on withdrawal and service-role-only aggregate reporting.
...
- Configure a Database Webhook on `member_notifications` INSERT to call
  `dispatch-push` with `x-push-secret`, plus EAS/Expo credentials. The sending
  worker is included; owner credentials are intentionally not committed.
...
- Apple App Store Server API / Google Play Developer API verifier adapter and
  approved product catalog. Follow `docs/BILLING-AND-LAUNCH-ANALYTICS.md`.
