...
select has_table('public','billing_verification_attempts','hashed store verification attempts are audited server-side');
select has_table('public','launch_analytics_sessions','consented launch sessions exist');
select has_table('public','launch_analytics_events','privacy-safe launch events exist');
select ok((select relrowsecurity from pg_class where oid='public.billing_verification_attempts'::regclass),'billing verification audit has RLS enabled');
select ok((select relrowsecurity from pg_class where oid='public.launch_analytics_sessions'::regclass),'launch sessions have RLS enabled');
select ok((select relrowsecurity from pg_class where oid='public.launch_analytics_events'::regclass),'launch events have RLS enabled');
select table_privs_are('public','billing_verification_attempts','authenticated',array[]::text[],'members cannot read or forge store verification evidence');
select table_privs_are('public','launch_analytics_sessions','authenticated',array[]::text[],'members access launch sessions only through bounded RPCs');
select table_privs_are('public','launch_analytics_events','authenticated',array[]::text[],'members access launch events only through bounded RPCs');
select has_function('public','start_launch_analytics_session',array['uuid','text','text','text'],'consented launch session RPC exists');
select has_function('public','record_launch_analytics_event',array['uuid','uuid','text','jsonb','timestamp with time zone'],'allowlisted launch event RPC exists');
select has_function('public','close_launch_analytics_session',array['uuid'],'launch session close RPC exists');
select has_function('public','get_launch_analytics_snapshot',array['timestamp with time zone'],'aggregate launch snapshot RPC exists');
select function_privs_are('public','start_launch_analytics_session',array['uuid','text','text','text'],'authenticated',array['EXECUTE'],'members may start only their own consented session');
select function_privs_are('public','record_launch_analytics_event',array['uuid','uuid','text','jsonb','timestamp with time zone'],'authenticated',array['EXECUTE'],'members may submit only allowlisted consented events');
select function_privs_are('public','close_launch_analytics_session',array['uuid'],'authenticated',array['EXECUTE'],'members may close only their own session');
select function_privs_are('public','get_launch_analytics_snapshot',array['timestamp with time zone'],'service_role',array['EXECUTE'],'launch aggregate is service-role only');
select ok(not has_function_privilege('anon','public.start_launch_analytics_session(uuid,text,text,text)','EXECUTE'),'anonymous clients cannot start launch analytics sessions');
...
select has_table('public','call_sessions','authenticated call lifecycle records exist');
select has_column('public','messages','delivered_at','messages store delivered timestamps');
select has_function('public','mark_match_messages_delivered',array['uuid'],'delivered receipt RPC exists');
select has_function('public','mark_match_messages_read',array['uuid'],'read receipt RPC exists');
select has_function('public','start_match_call',array['uuid','text','text'],'call start RPC exists');
select has_function('public','update_match_call',array['uuid','text','text','text'],'call update RPC exists');
select function_privs_are('public','mark_match_messages_delivered',array['uuid'],'authenticated',array['EXECUTE'],'members can mark only eligible incoming messages delivered');
select function_privs_are('public','mark_match_messages_read',array['uuid'],'authenticated',array['EXECUTE'],'members can mark only eligible incoming messages read');
select table_privs_are('public','call_sessions','authenticated',array['SELECT'],'members can only select RLS-filtered call records');
