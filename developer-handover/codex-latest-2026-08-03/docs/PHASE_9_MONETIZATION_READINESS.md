...
- Native `expo-iap` checkout and restore orchestration for Apple/Google, with
  transaction finishing allowed only after the server verifies and records the entitlement
- Authenticated `verify-store-purchase` Edge Function with HMAC verifier handoff,
  hashed receipt audit and cross-account receipt protection
- Exact Base/Plus/Elite monthly/annual, Spark 5/15/40 and Executive annual store
  mappings, shipped inactive until catalog evidence is approved
