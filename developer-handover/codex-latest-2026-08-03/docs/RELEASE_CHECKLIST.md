...
- [x] Source gate: native Apple/Google checkout/restore calls the server verifier
  before finishing transactions; raw receipt tokens are not stored
- [ ] Provider gate: configure Apple/Google credentials, approve catalog mappings
  and pass physical-device sandbox purchase/restore/refund evidence
...
- [x] Source gate: consented launch funnel, offline retry queue, property allowlist,
  withdrawal deletion and aggregate snapshot are implemented
- [ ] Operations gate: connect crash monitoring, approve analytics retention,
  assign dashboard/alert owners and pass consent QA on physical devices
