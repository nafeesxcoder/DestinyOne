...
export function buildFeatureFocusSnapshot(){
  ...
  const mainNavigationExperiments=primaryNavigation.filter(item=>!['qualified_introductions','supporting_paths','meaningful_conversation','date_planning','private_professional_matching','trust_and_control'].includes(item.purpose));
  return {
