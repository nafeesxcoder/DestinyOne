-- Use local names inside PL/pgSQL so the public RPC argument contract remains
-- unchanged without shadowing likes.recipient_id in queries or conflict targets.
create or replace function public.submit_match_decision(recipient_id uuid, decision text)
returns jsonb language plpgsql security definer set search_path=public as $$
declare
  viewer uuid := auth.uid();
  target_member_id uuid := submit_match_decision.recipient_id;
  normalized_decision text := lower(trim(submit_match_decision.decision));
