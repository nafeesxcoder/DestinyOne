...
begin
  ...
  if left(normalized_phone, 1) <> '+'
     or substring(normalized_phone from 2) !~ '^[1-9][0-9]{7,14}$' then
    raise exception 'Enter the complete phone number with country code';
  end if;
  ...
  if (select count(*) from public.couple_phone_search_audit where searcher_id = actor and created_at > now() - interval '1 hour') >= 10 then raise exception 'Phone search limit reached. Try again later'; end if;
