-- Keep the public RPC argument names stable while qualifying every table column.
-- PostgreSQL otherwise treats recipient_id as ambiguous inside PL/pgSQL queries.
...
begin
  ...
  if not exists (
    ...
    where r.user_id = viewer and r.target_id = recipient_id and r.recommendation_day = current_date
  ) and not exists (
    select 1 from public.likes l
    where l.sender_id = recipient_id and l.recipient_id = viewer and l.decision = 'interested'
  ) then
  ...
  select l.decision into previous_decision from public.likes l
  where l.sender_id = viewer and l.recipient_id = recipient_id;
  if previous_decision is distinct from normalized_decision then
    select count(*) into recent_count from public.discovery_signals ds
    where ds.user_id = viewer and ds.created_at >= now() - interval '24 hours';
  ...
  if normalized_decision = 'pass' then
    update public.matches m set status = 'passed'
    where m.status = 'suggested' and viewer in (m.user_a, m.user_b) and recipient_id in (m.user_a, m.user_b);
  ...
  if did_match then
    select exists (
      ...
      where viewer in (m.user_a, m.user_b) and recipient_id in (m.user_a, m.user_b) and m.status = 'mutual'
    ) into mutual_already;
