...
- twenty-six ordered migrations for server-owned profile/chat writes, reciprocal matching, preferences, matches,
...
- a manual GitHub production workflow with an environment approval gate, remote
  ...
  linked schema lint, the 199-assertion pgTAP suite, post-deploy verification,
...
- The legacy audit found only `profiles` and `messages` from an older 27-object
  inventory. It does not satisfy the current v26 contract of 78 tables and 59
...
7. Require linked lint and all 199 pgTAP assertions to pass, then require the
